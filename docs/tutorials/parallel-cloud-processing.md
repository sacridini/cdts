...
## 3. Why Zarr is Crucial (Say Goodbye to GeoTIFF)
...
def run_distributed_analysis():
    ...
    # This happens lazily - no processing starts yet!
    print("Mapping LandTrendr across the cluster...")
    lt_results = cube.cdts.run_landtrendr(years=years)
    
    # 4. Execute and stream the output to Cloud Storage in parallel
    # The workers will write their completed chunks directly to the Zarr bucket
    print("Executing distributed processing and saving...")
    lt_results.to_zarr('gs://my-bucket/LandTrendr_Results.zarr', mode='w')
